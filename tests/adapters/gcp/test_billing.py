...
class TestGetCost:
    ...
    def test_returns_cost_from_bigquery(self):
        ...
        with patch("adapters.gcp.billing._query_bigquery", return_value={"//compute.googleapis.com/projects/p/zones/z/instances/my-vm": 42.50}):
            result = get_cost(
                "my-proj",
        ...
        assert result["//compute.googleapis.com/projects/p/zones/z/instances/my-vm"] == 42.50

    def test_returns_zeros_on_bq_failure(self):
        rid = "//compute.googleapis.com/projects/p/zones/z/instances/vm1"
        with patch("adapters.gcp.billing._query_bigquery", side_effect=Exception("bq error")):
